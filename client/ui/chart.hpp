...
public:
  ...
  void update(msecu32_t const) override {
    if (_chan) {
      Packet<float> AccelXPkt;
      if (_chan->AccelX.pop(AccelXPkt)) {
        lv_chart_set_next_value(_base, _accelXSeries, 100.0 * AccelXPkt.value());
      }
      Packet<float> AccelYPkt;
      if (_chan->AccelY.pop(AccelYPkt)) {
        lv_chart_set_next_value(_base, _accelYSeries, 100.0 * AccelYPkt.value());
      }
      Packet<float> AccelZPkt;
      if (_chan->AccelZ.pop(AccelZPkt)) {
        lv_chart_set_next_value(_base, _accelZSeries, 100.0 * AccelZPkt.value());
      }
